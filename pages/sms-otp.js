// pages/sms-otp.js
// Single Next.js page: UI + server-side calls to Twilio Verify (SMS).
// No env vars. No API routes. No DB. Credentials are inline (testing only).
...
export default function SmsOtpPage({ initialPhone = '', statusMsg = '', approved = null, debug = null }) {
  return (
    <div style={S.wrap}>
      <div style={S.card}>
        <h1 style={{marginTop:0}}>SMS OTP Test (Twilio Verify)</h1>
        ...
        <form method="GET" style={{marginBottom:16}}>
          ...
          <button type="submit" style={S.btn}>Send SMS code</button>
          <div style={S.hint}>
            Verify Service (<code>VA…</code>) must allow <b>SMS</b> and your project must permit SMS to Italy in Geo Permissions.
          </div>
        </form>
...
export async function getServerSideProps(ctx) {
  ...
  // Helper: POST application/x-www-form-urlencoded to Twilio Verify
  ...
  try {
    if (action === 'send') {
      if (!isE164(phone)) {
        ...
        // channel=sms  --> sends OTP by SMS
        const r = await twilioPost(url, { To: phone, Channel: 'sms' });
        debug = { when: 'send', httpStatus: r.status, twilio: r.json };
        if (!r.ok) {
          statusMsg = `Error: ${r.json?.message || 'Twilio error'}`;
        } else {
          statusMsg = 'SMS sent. Check your phone for the 6-digit code.';
        }
      }
    }
