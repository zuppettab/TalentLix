...
const dobBoundsForAgeEq = (age) => {
  // birthdate in [today-(age+1)years + 1 day, today-age years]
...
const dobBoundForAgeGte = (age) => {
  // age >= n  => DOB <= today-n years
  return toISO(addYears(new Date(), -age));
};
const dobBoundForAgeLte = (age) => {
  // age <= n  => DOB >= (today-(n+1)years + 1 day)
  const min = addYears(new Date(), -age - 1);
...
// --- Styles (coerenti col progetto: sobrio/minimal) ---
const styles = {
  ...
  stageCard: {
    ...
    borderRadius: 20,
    padding: '1.35rem',
    boxShadow: '0 28px 60px -44px rgba(15,23,42,0.35)',
  },
  hero: { display: 'grid', gap: 12, maxWidth: 780, margin: '0 auto' },
  bigLabel: { fontSize: '1.6rem', fontWeight: 700, margin: 0, color: '#0f172a' },
  sub: { margin: 0, color: '#334155', fontWeight: 500 },
  bigSelect: { fontSize: '1rem' },
  ...
  layout: { display: 'grid', gap: 20, gridTemplateColumns: 'minmax(260px, 320px) minmax(0,1fr)' },
  filters: { display: 'grid', gap: 12, position: 'sticky', top: 12, alignSelf: 'start' },
  filterCard: {
  ...
  h3: { margin: 0, fontSize: '0.95rem', color: '#475569', fontWeight: 600 },
  radioRow: { display: 'flex', gap: 10, alignItems: 'center' },
  results: { display: 'grid', gap: 16, minWidth: 0 },
  resultsHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8, flexWrap: 'wrap' },
  ...
  card: {
    ...
    display: 'grid',
    gap: 10,
    boxShadow: '0 20px 54px -32px rgba(15,23,42,0.25)',
  },
...
export default function SupabaseAthleteSearchCard() {
  // --- Stage/state ---
  ...
  const [roles, setRoles] = useState([]);                  // array di string
  ...
  // --- Stage 1: check availability for selected sport ---
  const onContinue = async () => {
    ...
    try {
      ...
      if ((count || 0) < 1) {
        setNoData(`No athletes found for ${sport.label}.`);
        return;
      }
      setStage('search');
    } catch (e) {
  ...
  // --- Role suggestions (from DB, distinct) ---
  ...
  const fetchPage = async () => {
    ...
    try {
      let q = supabase
        .from('athlete')
        .select(`
          id, first_name, last_name, gender, nationality, date_of_birth, profile_picture_url,
      ...
      if (!Number.isNaN(n) && n >= 0 && ageMode) {
        if (ageMode === 'eq') {
          ...
          const maxDob = dobBoundForAgeGte(n);
          q = q.lte('date_of_birth', maxDob);
        } else if (ageMode === 'lte') {
          const minDob = dobBoundForAgeLte(n);
          q = q.gte('date_of_birth', minDob);
        }
      }
    ...
    return (
      <div style={styles.stageCard} aria-live="polite">
        <div style={styles.hero}>
          <h2 style={styles.bigLabel}>Select a sport</h2>
          ...
          <Select
            ...
            styles={{
              ...
              container: (b) => ({ ...b, fontSize: styles.bigSelect.fontSize }),
            }}
          />
      ...
      <div style={styles.layout}>
            ...
            <div>
              ...
              <div style={styles.radioRow}>
                ...
                <input
                  ...
                  style={{ height: 36, borderRadius: 8, border: '1px solid #E0E0E0', padding: '0 8px', width: 90 }}
                  placeholder="Years"
                />
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          setContractStatuses(prev =>
                            e.target.checked ? [...prev, cs.value] : prev.filter(v => v !== cs.value)
                          );
                        }}
                      /> {cs.label}
                    </label>
        ...
        <main style={styles.results} aria-live="polite">
          <header style={styles.resultsHeader}>
            <div>
              <h2 style={{ margin: 0, fontSize: '1.4rem', fontWeight: 700, color: '#0f172a' }}>Athletes</h2>
              <p style={styles.small}>Profiles update in real time as you adjust filters.</p>
            </div>
          ...
          <section style={styles.grid}>
            {rows.map((ath) => {
              ...
              const age = (() => {
                ...
                const dob = new Date(ath.date_of_birth);
                const now = new Date();
                ...
                <article key={ath.id} style={styles.card}>
                  ...
                  <div style={styles.factGrid}>
                    ...
                    <div style={{ gridColumn: '1 / -1' }}>
                      ...
                      {Array.isArray(exp?.preferred_regions) && exp.preferred_regions.length > 0
                        ? exp.preferred_regions.join(', ')
                        : '-'}
                    </div>
                  </div>
