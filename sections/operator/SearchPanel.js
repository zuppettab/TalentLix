...
// Router Next per sincronizzare query/filtri nell’URL
const routing = createInstantSearchNextRouter({ singletonRouter });

function Hit({ hit }) {
  // Non indicizziamo first/last_name (per scelta): mostriamo profilo sintetico coerente
  // Es: sport/ruolo/nationality/age + badge
  return (
    <article style={{
      border: '1px solid #eee', borderRadius: 12, padding: 12, display: 'grid', gap: 6
    }}>
      <div style={{fontSize: 12, opacity: 0.7}}>ID #{hit.objectID}</div>
      <div><strong><Highlight attribute="role" hit={hit} /></strong> · <Highlight attribute="sport" hit={hit} /></div>
...
export default function SearchPanel() {
  return (
    <>
      <Head>
        {/* Favicon come da standard del progetto */}
