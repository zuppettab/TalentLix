...
const buildConfigError = (snapshot) => {
  const error = createHttpError(500, 'Supabase client is not configured.', {
...
export const resolveAuthenticatedRequestContext = async (accessToken, options = {}) => {
  if (!accessToken) {
  ...
  if (!candidateClients.length || (requireServiceRole && !serviceClient)) {
  ...
  for (const { label, client } of candidateClients) {
    try {
      ...
      if (error) {
        console.error(`Failed to verify session via ${label} client`, error);
        continue;
      }
      if (data?.user) {
        user = data.user;
      ...
      console.error(`Failed to verify session via ${label} client`, authError);
    }
  }
...
export default resolveAuthenticatedRequestContext;
